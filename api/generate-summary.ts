...
// FIX: Extracted schema generation to a constant outside the handler for clarity and performance.
...
// FIX: Extracted prompt generation into a helper function to resolve scoping issues.
const getPrompts = (targetLanguage: string, userData: UserData): Record<string, string> => {
...
export default async function handler(request: Request) {
  ...
  try {
    ...
    const prompts = getPrompts(targetLanguage, userData);
    ...
    JSON.parse(resultJson); // Sanity check to ensure the response is valid JSON before sending
