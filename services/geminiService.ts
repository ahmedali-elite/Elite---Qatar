...
// In a real app, this function would send data to your server.
export const sendDataToBackend = (userData: UserData, summaryData: SummaryData, t: (key: string, options?: any) => string) => {
    // For now, we'll format it for a mailto link as a simple lead capture mechanism.
...
export const getFullJourneySummary = async (userData: UserData, language: 'en' | 'ar'): Promise<SummaryData> => {
    console.log(`Starting summary generation for language: ${language}`);
    ...
    try {
        ...
        console.log("Successfully generated English plan.");

    } catch (error) {
        console.error("Error generating English strategic plan:", error);
        return getDefaultSummary(); // Fallback on failure
    }

    // --- STEP 2: If language is Arabic, translate the English plan ---
    if (language === 'ar') {
        console.log("Language is Arabic, proceeding with translation.");
        try {
